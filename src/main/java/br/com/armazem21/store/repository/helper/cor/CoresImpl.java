...
public class CoresImpl implements CoresQueries {
	...
	public Page<Cor> filtrar(CorFilter filtro, Pageable pageable) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Cor.class);
	...
	private Long total(CorFilter filtro) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Cor.class);
		adicionarFiltro(filtro, criteria);
	...
	private void adicionarFiltro(CorFilter filtro, Criteria criteria) {
		if (filtro != null) {
